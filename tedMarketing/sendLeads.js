...
async function main() {
    ...
    for ( let i = start; i < end; i++) {
        let element = array[i];
        // p(element);
        form = buildForm('facebook-it', element, 3);
        // form = buildForm('facebook', element, 3);
        // form = buildForm('firestore', element, 3);
        options.form = form;
        options.form.campaign = 'Kunal 11';
        p(options.form);
        // /*
        try {
            res = await rp(options);
            p(res);
            await sleep(Math.floor(Math.random() * 500000));
        } catch (error) {
...
function buildForm(source, element, counter) {
    switch(source) {
        ...
        case 'facebook-it' : {
            // p(element['created_time']);
            return {
                'name': element['nome_e_cognome'],
                'campaign': `dempsey's salon`,
